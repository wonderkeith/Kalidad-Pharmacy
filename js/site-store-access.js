...
function isContactOrNewsOrAbout() { const n = pageName(); return n.includes("contact") || n.includes("news") || n.includes("about"); }
...
function addStyles() {
  ...
  style.textContent = `
    ...
    .kalidad-hide-when-authenticated{display:none!important}
    .kalidad-hide-open-hours{display:none!important}
    .kalidad-hide-place-order{display:none!important}
...
function makeLink(text, href, className) {
  const a = document.createElement("a"); a.href = relative(href); a.textContent = text;
  a.className = `kalidad-store-link ${className || ""}`; return a;
}
...
function mountStoreLinks() {
  ...
  const accountExcluded = isCareersOrAbout();
  const cartExcluded = isContactOrNewsOrAbout();
  const loggedIn = !!auth.currentUser;

  [nav, mobile].forEach(container => removeDuplicateLinks(container, ["Shop Online", "My Account", "Cart", "Cart (0)"]));

  // Shop Online is the public sign-in entry point. Once authenticated it disappears.
  if (loggedIn) {
    removeLinks(nav, ["Shop Online"]);
    removeLinks(mobile, ["Shop Online"]);
  } else {
    if (nav) ensureLink(nav, "Shop Online", LOGIN_URL, "kalidad-shop-link");
    if (mobile) {
      const oldShop = findLink(mobile, ["Shop Online"]);
      if (oldShop) oldShop.href = relative(LOGIN_URL);
      else mobile.appendChild(makeLink("Shop Online", LOGIN_URL, "kalidad-shop-link"));
    }
  }

  // Authenticated customers get My Account on every public page except Careers/About.
  if (loggedIn && !accountExcluded) {
    if (nav) ensureLink(nav, "My Account", ACCOUNT_URL, "kalidad-account-link");
  } else {
    removeLinks(nav, ["My Account"]);
  }

  // Cart remains on other pages, but is hidden for authenticated customers on Contact/News/About.
  if (loggedIn && cartExcluded) {
    removeLinks(nav, ["Cart", "Cart (0)"]);
  } else if (!home) {
    if (nav) ensureLink(nav, "Cart (0)", CART_URL, "kalidad-cart-link");
  } else {
    removeLinks(nav, ["Cart", "Cart (0)"]);
  }
  ...
  if (mobile) {
    ...
    if (!loggedIn) group.appendChild(makeLink("Shop Online", LOGIN_URL, "kalidad-shop-link"));
    if (loggedIn && !accountExcluded) group.appendChild(makeLink("My Account", ACCOUNT_URL, "kalidad-account-link"));
    if (!home && !(loggedIn && cartExcluded)) group.appendChild(makeLink("Cart (0)", CART_URL, "kalidad-cart-link"));
    mobile.appendChild(group);
  }
}

function applyAuthenticatedCleanup() {
  const loggedIn = !!auth.currentUser;
  const contactPage = pageName().includes("contact");

  // Contact page: remove Open 24/7 regardless of login state.
  if (contactPage) {
    document.querySelectorAll(".open-badge, [class*='open-badge']").forEach(el => el.remove());
    [...document.querySelectorAll("span, a, button, div")].forEach(el => {
      if (normalizedText(el.textContent) === "open 24/7" || normalizedText(el.textContent) === "24/7") {
        if (el.children.length === 0 || el.tagName === "SPAN") el.remove();
      }
    });
  }
  ...
  // Logged-in state: remove Shop Online, 24/7 badge/span and Place Your Order everywhere.
  document.querySelectorAll("nav a, nav button").forEach(el => {
    const text = normalizedText(el.textContent);
    if (text === "shop online") el.remove();
    if (text === "place your order" || text === "place an order" || text === "order now") el.remove();
  });

  document.querySelectorAll(".open-badge, [class*='open-badge']").forEach(el => el.remove());
  [...document.querySelectorAll("span, a, button")].forEach(el => {
    const text = normalizedText(el.textContent);
    if (text === "open 24/7" || text === "24/7" || text === "place your order" || text === "place an order") el.remove();
  });
}
...
function mount() {
  addStyles(); mountStoreLinks(); mountStaffLogin(); updateCartCount(); preparePrescriptionTarget(); applyAuthenticatedCleanup();
...
if (!location.pathname.includes("/store/") && !location.pathname.includes("/admin/")) {
  mount();
  onAuthStateChanged(auth, () => { mountStoreLinks(); updateCartCount(); applyAuthenticatedCleanup(); });
}
