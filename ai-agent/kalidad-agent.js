...
(function () {
  ...
  css.textContent = `
    ...
    .ka-panel{position:absolute;right:0;bottom:74px;width:min(410px,calc(100vw - 28px));height:min(680px,calc(100vh - 110px));background:#F6F4EC;border:1px solid #E4E1D6;border-radius:24px;overflow:hidden;box-shadow:0 24px 70px rgba(18,41,31,.28);display:none;flex-direction:column}
    ...
    .ka-form{display:flex;gap:8px;padding:12px 14px;border-top:1px solid #E4E1D6;background:#fff}.ka-input{min-width:0;flex:1;border:1px solid #D8D6CC;border-radius:14px;padding:11px 12px;font:inherit;font-size:.84rem;outline:none}.ka-input:focus{border-color:#1E4F3B}.ka-send{width:44px;border:0;border-radius:13px;background:#1E4F3B;color:#fff;cursor:pointer}.ka-send:disabled{opacity:.55;cursor:not-allowed}.ka-note{padding:0 16px 9px;font-size:.62rem;color:#647166;background:#fff;text-align:center}
    .ka-status{font-size:.67rem;color:#647166;text-align:center;padding:5px 16px 10px;background:#fff}
    @media(max-width:600px){#kalidad-ai{right:14px;bottom:14px}.ka-panel{right:-2px;bottom:70px;height:min(700px,calc(100vh - 94px));border-radius:22px}.ka-launch{width:56px;height:56px}}
  `;
  ...
  root.id = 'kalidad-ai';
  root.innerHTML = `
    <section class="ka-panel" aria-label="Kalidad AI Pharmacy Assistant" role="dialog" aria-modal="false">
      <header class="ka-head">
        <div class="ka-brand"><div class="ka-mark">K</div><div><div class="ka-title">Kalidad AI Pharmacy Assistant</div><div class="ka-sub">General guidance • Pharmacist escalation available</div></div></div>
      ...
      <div class="ka-quick">
        ...
        <button data-q="Help me explore Kalidad's wellness categories">🛒 Shop</button>
      ...
      <div class="ka-status" aria-live="polite"></div>
      <div class="ka-note">For safety, this assistant does not replace a pharmacist or doctor.</div>
      <form class="ka-form"><input class="ka-input" autocomplete="off" maxlength="4000" placeholder="Ask Kalidad anything…" aria-label="Message"/><button class="ka-send" aria-label="Send" type="submit">➤</button></form>
    </section>
  ...
  var panel = root.querySelector('.ka-panel');
  var launch = root.querySelector('.ka-launch');
  var close = root.querySelector('.ka-close');
  var messages = root.querySelector('.ka-messages');
  var form = root.querySelector('.ka-form');
  var input = root.querySelector('.ka-input');
  var sendButton = root.querySelector('.ka-send');
  var status = root.querySelector('.ka-status');
  var history = [];
  var busy = false;

  function add(text, who, extraClass) {
    var el = document.createElement('div');
    el.className = 'ka-msg ' + who + (extraClass ? ' ' + extraClass : '');
    el.textContent = text;
  ...
  add('Hello 👋🏾 I’m Kalidad’s AI pharmacy assistant. I can help with general health questions, wellness categories, pharmacy services, and pharmacist handoff. What can I help you with?', 'bot');

  function open() { panel.classList.add('open'); input.focus(); }
  function setBusy(value) { busy = value; sendButton.disabled = value; input.disabled = value; status.textContent = value ? 'Connecting to Kalidad AI…' : ''; }
  ...
  async function send(text) {
    ...
    var wait = add('Thinking…', 'bot', 'wait');
    setBusy(true);
    try {
      ...
      var reply = data.reply || 'I’m unable to answer that right now. Please speak with a Kalidad pharmacist.';
      wait.textContent = reply;
      wait.classList.remove('wait');
      history.push({ role: 'assistant', content: reply });
    } catch (error) {
      wait.textContent = 'I’m currently unable to reach the AI service. Please use the pharmacist/WhatsApp option or try again shortly.';
      wait.classList.remove('wait');
    } finally {
      setBusy(false);
  ...
  root.querySelectorAll('.ka-quick button').forEach(function(button){
    button.addEventListener('click', function(){ send(button.dataset.q); });
  });
})();
